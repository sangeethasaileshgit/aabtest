            ...
            builderDelegate: PagedChildBuilderDelegate<RefundRequestDetails>(
              itemBuilder: (_, item, _) {
                ...
                return DynamicListCard.keyValueBody(
                  ...
                  title: '${context.t.common.invoiceId}: ${item.deposit?.invoiceNo ?? "N/A"}',
                  ...
                  keyValues: [
                    ...{
                      ...
                      context.t.common.securityDeposit: item.deposit?.depositAmount?.quickCurrency() ?? "N/A",
                      context.t.common.utilityBill: item.deposit?.utilityAdvance?.quickCurrency() ?? "N/A",
                      context.t.common.rentalAdvance: item.deposit?.rentAdvance?.quickCurrency() ?? "N/A",
                      context.t.common.totalAmount: item.deposit?.totalAmount?.quickCurrency() ?? "N/A",
                      context.t.common.refundReason: item.refundReason ?? "N/A",
                      context.t.common.status: _status,
                      context.t.common.note: context.t.exceptions.refundRequestHint.inTenantList,
                    }.entries.map((entry) {
      ...
      FormWrapper(
        builder: (formContext) {
          return InvoiceDetailsView.printInvoice(
                        ...
                        if (FormWrapper.validate(formContext)) {
                          final _result = await showAsyncLoadingOverlay(
                            ...
                            asyncFunction: () => Future.microtask(
                              () => ref.read(paymentsRepoProvider).approveRefundPayment(data.id!),
                            ),
                          );
            ...
            scrollbodyWidget: _status.isPaid || (_status.isUnpaid && !data.hasReceipt)
                ? null
                : Column(
