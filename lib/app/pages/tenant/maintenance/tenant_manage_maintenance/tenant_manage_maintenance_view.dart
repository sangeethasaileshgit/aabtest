...
class TenantManageMaintenanceView extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<TenantManageMaintenanceView> createState() => _TenantManageMaintenanceViewState();
}

class _TenantManageMaintenanceViewState extends ConsumerState<TenantManageMaintenanceView> {
  @override
  void initState() {
    if (widget.isEditing) {
      ref.read(tenantManageMaintenanceViewProvider).handleInitEdit(widget.editModel!);
    }
    super.initState();
  }
                    ...
                    builder: (_) {
                      ...
                      if (_myRentResp.hasError && !_myRentResp.hasValue) {
                        return Text.rich(
                          RetryButtons.inlineText(
                            _myRentResp.error!,
                            onAction: () => ref.refresh(propertyListProvider(null)),
                          ),
                        );
                      }
                      final _properties = [...?_myRentResp.valueOrNull?.data?.data];
                      ...
                      return Skeletonizer(
                        ...
                        child: DropdownButtonFormField<int>(
                          ...
                          hint: Text(
                            context.t.form.anyDropdown.label(label: context.t.common.property).sentenceCase,
                          ),
                          decoration: InputDecoration(
                            // labelText: 'Property Name',
                            labelText: context.t.form.anyDropdown.label(label: context.t.common.propertyName),
                          ),
                          isExpanded: true,
                          initialValue: controller.selectedPropertyId,
                          items: List.generate(
                            _properties.length,
                            (index) {
                              ...
                              return DropdownMenuItem<int>(
                                ...
                                child: Text(
                                  _property.property?.stepTwoData?.adTitle ?? "N/A",
                                ),
                              );
                            },
                          ),
          ...
          bottomNavigationBar: ScaffoldBottomNavWrapper(
            child: SizedBox(
              ...
              child: ElevatedButton(
                ...
                child: Text(
                  widget.isEditing ? context.t.action.update : context.t.action.sendRequest,
                ),
              ),
  ...
  Future<void> _handleFormSubmit(BuildContext ctx) async {
    final _result = await showAsyncLoadingOverlay(
      ...
      asyncFunction: () => Future.microtask(
        () => ref.read(tenantManageMaintenanceViewProvider).handleFormSubmit(widget.editModel?.id),
      ),
    );
...
final propertyListProvider = FutureProvider.autoDispose.family<PaginatedRentListModel, String?>(
