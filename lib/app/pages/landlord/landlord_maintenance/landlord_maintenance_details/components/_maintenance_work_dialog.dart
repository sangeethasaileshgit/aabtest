...
class MaintenanceWorkDialog<T> extends StatefulWidget {
  ...
  @override
  State<MaintenanceWorkDialog<T>> createState() => _MaintenanceWorkDialogState<T>();
}
                ...
                child: SingleChildScrollView(
                  child: Column(
                    ...
                    children: [
                      // Header
                      Text(
                        context.t.common.maintenanceWork,
                          ...
                          if (_laborsResp.hasError && !_laborsResp.hasValue) {
                            return Text.rich(
                              RetryButtons.inlineText(
                                _laborsResp.error!,
                                onAction: () => ref.refresh(laborListProvider),
                              ),
                            );
                          }
                          final _labors = [...?_laborsResp.valueOrNull?.data?.data];

                          return Skeletonizer(
                            enabled: _laborsResp.isAnyLoading,
                            child: DropdownButtonFormField<int>(
                              hint: Text(context.t.common.selectLabor),
                              decoration: InputDecoration(
                                labelText: context.t.common.selectLabor,
                              ),
                              isExpanded: true,
                              initialValue: selectedLaborId,
                              items: List.generate(
                                _labors.length,
                                (index) {
                      ...
                      NumberFormField(
                        ...
                        decoration: InputDecoration(
                          labelText: context.t.common.maintenanceCost,
                          hintText: context.t.common.enterMaintenanceCost,
                        ),
                        textInputAction: TextInputAction.next,
                        validator: (value) {
                          return NumberFormField.nonZeroRequired(
                            value,
                            emptyErrorText: context.t.common.pleaseEnterMaintenanceCost,
                          );
                        },
                      ),
                      const SizedBox.square(dimension: 16),
                      ...
                      TextFormField(
                        ...
                        decoration: InputDecoration(
                          labelText: context.t.common.comment,
                          hintText: context.t.common.writeComment,
                      ...
                      child: ElevatedButton(
                        onPressed: () {
                          if (Form.maybeOf(formContext)?.validate() ?? false) {
                            final _cost = maintenanceCostController.getNumber ?? 0;
                        ...
                        child: Text(context.t.action.done),
                      ),
                    ),
                  ),
                ],
...
typedef MaintenanceWorkDialogData = ({int laborId, num maintenanceCost, String comment});
