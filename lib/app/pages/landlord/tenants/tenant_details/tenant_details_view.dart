...
class _TenantDetailsViewState extends ConsumerState<TenantDetailsView> {
          ...
          child: ListTile(
            ...
            leading: SizedBox.square(
              ...
              child: CustomNetworkImage(
                url: _tenantDetails.valueOrNull?.image?.remote,
              ),
            ),
            title: Text(_tenantDetails.valueOrNull?.name ?? 'N/A'),
            titleTextStyle: _theme.textTheme.bodyLarge?.copyWith(
              fontWeight: FontWeight.w600,
              color: _theme.colorScheme.onPrimary,
            ),
            subtitle: Text(_tenantDetails.valueOrNull?.uniqueId ?? 'N/A'),
            ...
            child: _tenantDetails.when(
              ...
              error: (error, _) {
                return RetryButtons.scrollView(
                  error,
                  onAction: () => ref.refresh(landlordTenantDetails(widget.id)),
                );
              },
              loading: () {
                return Skeletonizer(
  ...
  Future<bool?> _handleCancel(BuildContext ctx, RentDetails? data) async {
    ...
    return await ctx.router.pushWidget<bool>(
      CancelRentView(
        id: data!.id!,
    ...
    if (id == null) {
      showCustomSnackBar(
        ...
        content: Text(
          ...
          context.t.exceptions.invalidApiDataRefreshPage(dataType: context.t.common.rent),
        ),
