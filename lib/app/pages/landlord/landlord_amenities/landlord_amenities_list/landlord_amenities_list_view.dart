...
@RoutePage()
class LandlordAmenitiesListView extends ConsumerStatefulWidget {
  const LandlordAmenitiesListView({super.key});

  @override
  ConsumerState<LandlordAmenitiesListView> createState() => _LandlordAmenitiesListViewState();
}

class _LandlordAmenitiesListViewState extends ConsumerState<LandlordAmenitiesListView>
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: CustomAppBar(title: Text(context.t.common.amenitiesList)),
                  ...
                  builderDelegate: PagedChildBuilderDelegate<repo.FacilityAmenity>(
                    itemBuilder: (_, item, _) {
                      return ItemAttributeListTile(
                        ...
                        onEdit: () async {
                          return context.router.push<void>(
                            LandlordManageAmenitiesRoute(editModel: item),
                          );
                        },
    ...
    try {
      await showAsyncLoadingOverlay(
        ...
        asyncFunction: () => Future.microtask(
          () => ref.read(repo.propertyRepoProvider).deleteAmenity(id),
        ),
      );
    } catch (e) {
      if (context.mounted) {
  ...
  Future<repo.PaginatedListModel<repo.FacilityAmenity>> fetchData(int page) {
    return ref.read(repo.propertyRepoProvider).getAmenities(page: page, search: searchController.text);
  }

  repo.EventSub<repo.AmenityEvent>? _apiEventSub;
  @override
  void initRefreshListener() {
    _apiEventSub = repo.gEventListener.on<repo.AmenityEvent>().listen((_) {
