...
abstract class SharedWidgets {
        ...
        if (_paymentResult.isFailure) {
          final didRetry = await ctx.router.pushWidget<bool>(
            PaymentStatusView(
              ...
              status: PaymentStatusViewType.fail.copyWith(
                description: _paymentResult.left,
              ),
            ),
          );
      ...
      try {
        ...
        if (ctx.mounted) {
          showCustomSnackBar(
            ...
            content: Text(ctx.t.exceptions.errorOpeningFile(error: e.toString())),
            customSnackBarType: CustomOverlayType.error,
          );
        }
      }
  ...
  static Future<void> handleChangePropertyStatus(
    ...
    if (context.mounted) {
      ...
      showCustomSnackBar(
        ...
        customSnackBarType: _response.isError ? CustomOverlayType.error : CustomOverlayType.success,
      );
      return;
    }
  }
  ...
  //--------------Handle Property Wish listing--------------//
  static Future<void> handleFavTap(
    BuildContext context,
    PropertyModel data, {
    bool isRemove = false,
  }) async {
    try {
      final _repo = ProviderScope.containerOf(context).read(propertyRepoProvider);
      final _exec = isRemove ? _repo.deleteFavorite(data.favorite!.id!) : _repo.createFavorite(data.id!);
