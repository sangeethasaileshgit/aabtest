...
class SubscriptionListView extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<SubscriptionListView> createState() => _SubscriptionListViewState();
}
          ...
          return await ctx.router.pushWidget<void>(
            PaymentStatusView(
              ...
              status: PaymentStatusViewType.custom(
                ...
                description: context.t.pages.subscriptionPlan.extra.subscriptionPaymentSuccess,
              ),
            ),
          );
        }
      ...
      child: EmptyWidget(
        ...
        emptyBuilder: (context) {
          return RetryButtons.scrollView(
            error,
            onAction: () => ref.refresh(subscrptionPlanProvider.future),
          );
        },
      ),
    );
...
class PricingCardWidget extends StatelessWidget {
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Text.rich(
            TextSpan(
              text: "${(data.offerPrice ?? data.subscriptionPrice)?.quickCurrency() ?? "N/A"} ",
              children: [
          ...
          ...{
            ...
            (feature) {
              ...
              return _buildFeatures(
                context,
                Feature(
                  name: "${_isUnlimited ? "Unlimited" : "${feature.value ?? 0}"} ${feature.key}",
  ...
  Widget _buildFeatures(BuildContext ctx, Feature feature) {
    ...
    final _iconColor = feature.isAvailable ? _theme.colorScheme.primary : DAppColors.kRejectedColor;
