...
class _ContactUsViewState extends ConsumerState<ContactUsView> {
                  ...
                  data: (data) {
                    ...
                    return ListView.separated(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        final sender = user?.id == chatData.senderId ? 'user' : 'admin';
                        ...
                        onPressed: isSending
                            ...
                            : () async {
                                return await _handleSendingMessage(
                                  context,
                                  isAttachment: true,
                                  parentId: supportSession.valueOrNull?.data?.firstOrNull?.id,
                                );
                              },
                        icon: const Icon(Icons.add),
                        style: IconButton.styleFrom(
                          backgroundColor: _theme.colorScheme.secondary.withValues(
                            alpha: 0.15,
                          ),
  ...
  Future<void> _handleSendingMessage(
    ...
    final result = await Future.microtask(
      () => (isAttachment ? prov.handleSendingAttachment(parentId) : prov.sendMessage(parentId)),
    );
