...
class TenantProfileFormFieldNotifier extends ChangeNotifier {
  ...
  // Nominee
  late final nomineeSectionController = ExpansibleController();
  late final nomineeNameController = TextEditingController();
  late final nomineeEmailController = TextEditingController();
  CountryCode? nomineeSelectedCountryCode = PhoneFormField.countryCodes.firstOrNull;
  ...
  // Emergency Contact
  late final ecSectionController = ExpansibleController();
  ...
  late final cCompanySectionController = ExpansibleController();
  late final cCompanyNameController = TextEditingController();
  late final cCompanySSMController = TextEditingController();

  // Workplace
  late final wCompanySectionController = ExpansibleController();
  ...
  // Vehicles
  late final wVehiclesSectionController = ExpansibleController();
  ...
  final List<DynamicFileType> nidPassportImages = List<DynamicFileType>.generate(
    2,
    (i) => DynamicFileType(),
  );
  ...
  void initEdit(User? user) {
    ...
    nomineeMobileNumberController.text = user.userDetails?.nomineeInfo?.phone?.mobileNo ?? '';
    ...
    ecMobileNumberController.text = user.userDetails?.emergencyContact?.phone?.mobileNo ?? '';

    // Company Info
    cCompanyNameController.text = user.userDetails?.companyInfo?.companyName ?? '';
    cCompanySSMController.text = user.userDetails?.companyInfo?.companySsmNo ?? '';

    // Workplace
    wCompanyNameController.text = user.userDetails?.workplace?.companyName ?? '';
    wCompanyAddress1Controller.text = user.userDetails?.workplace?.addressOne ?? '';
    wCompanyAddress2Controller.text = user.userDetails?.workplace?.addressTwo ?? '';
    wSelectedCountry = user.userDetails?.workplace?.country;
    wCompanyStateController.text = user.userDetails?.workplace?.state ?? '';
    wCompanyPostalCodeController.text = user.userDetails?.workplace?.postalCode ?? '';
    wCompanyCityController.text = user.userDetails?.workplace?.city ?? '';
    wCompanyOfficePhoneController.text = user.userDetails?.workplace?.officePhone ?? '';
    ...
    wCompanyOfficeMobileController.text = user.userDetails?.workplace?.phone?.mobileNo ?? '';
