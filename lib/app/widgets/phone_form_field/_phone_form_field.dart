...
class _CountryCodePicker extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (selectedCountry == null && _country != null) {
      WidgetsBinding.instance.addPostFrameCallback((_) {
        return onSelect?.call(_country);
      });
    }
              ...
              children: [
                ...
                TextField(
                  onChanged: (value) => setState(() {
                    _filteredItems = _filteredItems.where((element) {
                      return [
                        element.name,
                        element.dialCode,
                      ].whereType<String>().join().trim().toLowerCase().contains(value.trim().toLowerCase());
                    }).toList();
                  }),
