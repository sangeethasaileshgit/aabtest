    ...
    final _result = await showDialog<bool>(
      ...
      builder: (popContext) {
        return InfoDialog.confirmation(
          ...
          description: description ?? 'Fields that are changed may not be saved!',
...
class DefaultTabControllerWrapper extends StatelessWidget {
  const DefaultTabControllerWrapper({
    super.key,
  ...
  // ignore: library_private_types_in_public_api
  final _TabControllerWrapperBuilder builder;
  ...
  static TabController? maybeOf(BuildContext tabContext) {
    return DefaultTabController.maybeOf(tabContext);
  }

  static TabController of(BuildContext tabContext) {
    return DefaultTabController.of(tabContext);
  }
}

typedef _TabControllerWrapperBuilder = Widget Function(BuildContext tabContext);
