  ...
  Future<Either<String, User?>> getUser() async {
    try {
      ...
      if (_response.statusCode == HttpStatus.ok) {
        ...
        CurrencyService.updateSymbol(_model.data?.systemCurrency ?? "RM");
  ...
  Future<Either<String?, OtpSubmitModel?>> submitOtp({
    ...
    try {
      ...
      if (_response.statusCode == HttpStatus.ok) {
        ...
        final _token = _data.token ??
            (_response.data is Map<String, dynamic>
                ? (_response.data['data']?['token'] as String?)
                : null);

        if (saveToken != null && _token != null) {
          await httpClient.setToken(_token, saveToken == true);
        }
...
final countryProvider = FutureProvider<List<Country>>((ref) {
