...
extension NumberFormatterExtension on num {
  ...
  String quickCurrency({
    ...
    final _symbol = customCurrency ?? CurrencyService.symbol;
  ...
  String compactCurrency({
    ...
    final symbol = customCurrency ?? CurrencyService.symbol;
...
extension StringFormatterExtension on String {
  ...
  String _groupWithGap(String str, int groupCount, int? gapCount) {
    ...
    for (int i = 0; i < str.length; i += groupCount) {
      groups.add(
        str.substring(i, i + groupCount > str.length ? str.length : i + groupCount),
      );
    }
...
extension IterableExt<T> on Iterable<T> {
  Iterable<T> removeWhereX(bool Function(T element) predicate) {
    return where((element) => !predicate(element));
  }
}
